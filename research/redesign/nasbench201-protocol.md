...
## Recorded execution correction

The first full timing panel reached TorchDynamo's recompilation limit across fresh
workflow instances. Its default partial-graph compilation allowed eager fallback.
Reject its compiled speed labels even though all recorded scores and counts passed.
Preserve that complete artifact and its independent value audit.

The single correction resets the in-process compiler before each independent
compiled workflow and requires `fullgraph=True`. Record the reset cost outside
search timing. Require at least one captured graph after the first compiled
generation and no additional graphs during the 48 steady generations.
Report fresh-workflow tracing cost separately. The disk compiler cache may be
reused within the panel. Start the corrected panel with a new cache directory.

Rerun every timing configuration. Do not change algorithms, budgets, data, scoring,
or the completed quality panel. Retain the [initial protocol](results/nasbench201/protocol-initial.md)
whose hash appears in the quality results.
